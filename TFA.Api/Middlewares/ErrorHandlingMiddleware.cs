...
public class ErrorHandlingMiddleware(RequestDelegate next)
{
    ...
    ProblemDetailsFactory problemDetailsFactory)
    {
        try
        {
            logger.LogInformation("Error handling started for request in the path {RequestPath}", httpContext.Request.Path.Value);
            
            await next.Invoke(httpContext);
        }
        catch (Exception exception)
        {
            logger.LogError(exception, "Error has happened with {RequestPath}, the message is {ErrorMessage}", httpContext.Request.Path.Value, exception.Message);
            ...
            switch (exception)
            {
                ...
                case  DomainException domainException:
                    ...
                    break;
                default:
                    problemDetails = problemDetailsFactory.CreateProblemDetails(httpContext, StatusCodes.Status500InternalServerError, "Unhandled error! Please contact us.");
